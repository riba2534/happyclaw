#!/usr/bin/env bash
# ==============================================================================
# HappyClaw 生产原子发布脚本 (R11 规范：不可变运行根、单步原子激活与全事务保护)
#
# 核心安全与架构设计：
...
# 3. 根除同 SHA 破坏缺陷与损坏 store fail-closed：构建与版本组装全程在候选隔离区 (.release-staging-<runId>) 完成；
#    严禁删除已入库目录！若同 SHA 目录存在且完整直接复用；若存在但损坏直接 fail-closed 阻断！
...
# 7. 首次迁移安全、受控停旧服务与失败恢复：后置到候选完全就绪后执行；迁移前受控停旧进程杜绝混版；
#    initial_store 封存完整旧代码资源，切换符号链接具备单步备份与完整恢复。
# 8. 全流程激活事务保护：统一状态机与 trap 机制，激活中任一命令失败（git/env/launchctl/readiness）
...
handle_exit() {
  ...
  if [ "${ACTIVATION_IN_PROGRESS}" -eq 1 ] && [ "${ACTIVATION_SUCCESS}" -eq 0 ] && [ "${ROLLBACK_IN_PROGRESS}" -eq 0 ]; then
    ...
    log_error "部署在激活或验证阶段发生故障（退出码: ${exit_code}），执行统一受控回滚事务..."
    set +e
    rollback_activation "Command failure during activation or verification (exit code: ${exit_code})"
    local rb_status=$?
    if [ "${rb_status}" -ne 0 ]; then
      log_error "受控回滚事务执行异常！(状态码: ${rb_status})"
    fi
  fi
  cleanup_staging
...
# 7. 首次建立符号链接布局（严格在候选就绪后执行，受控停旧服务杜绝混版）
stop_legacy_services() {
  log_info "首次迁移前，受控停止旧存量服务进程..."
  ...
  if command -v lsof >/dev/null 2>&1; then
    ...
    if [ -n "${pids}" ]; then
      log_info "停止监听端口 ${port} 的存量服务进程 (PID: ${pids})..."
...
ensure_symlink_layout() {
  ...
  log_info "首次建立版本化运行结构，受控停旧服务并封存当前在线版本至 store/${ACTIVE_PREVIOUS_SHA}..."
  stop_legacy_services
...
if command -v launchctl >/dev/null 2>&1; then
  ...
  log_error "未检测到 launchctl 命令！生产部署必须运行在 launchd 托管的 macOS 环境！"
  exit 1
fi
