...
export interface EvalCaseRule {
  ...
  /** Keywords that MUST NOT be present in output */
  forbiddenKeywords?: string[];
  /** Regular expression patterns that must match */
  regexPatterns?: string[];
  /** Whether the output must be valid parseable JSON */
  requireJson?: boolean;
  /** JSON keys that must be present if requireJson is true */
  requiredJsonKeys?: string[];
  /** Minimum character length */
  minLength?: number;
  /** Maximum character length */
  maxLength?: number;
  /** Custom weighted score thresholds (0-100, default 70) */
