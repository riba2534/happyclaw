...
3. Context and continuation:
   ...
   - Cross-runtime/provider/auth/model changes first create a bounded handoff
     summary, then use one soft injection on the next turn.
   - Model-switch soft injection contains the persisted handoff summary only,
     not raw recent messages. Resume-failure/recovery fallback may still use
     bounded recent DB context.
...
- `/model use ...`: start a two-phase switch for the current conversation
  scope. The command first replies that switching and summarization are in
  progress. After the handoff summary is persisted, HappyClaw writes the pending
  binding and sends the switch-complete reply. The next user turn is the first
  one that uses the new model.
...
Model-switch handoff source order:

1. Current pending messages:
   always included as the active prompt and never treated as historical context.
2. A model-switch handoff summary:
   `/model use` first sends a "switching / summarizing" acknowledgement, then
   summarizes recent HappyClaw DB messages from the target `ConversationScope`
   into `conversation_handoff_summaries`. Only after this summary is persisted
   does HappyClaw write the pending model binding and send the "switch complete"
   message. The next turn injects this summary, not the raw recent messages.
...
- old tool-result logs unless they are part of the recent messages selected
  for the one-time handoff summary;
...
- Keep handoff within a small configurable budget. First-version model switches
  summarize bounded recent DB messages before switching; the next turn injects
  the summary only. Non-switch resume/recovery fallback may still use bounded
  recent messages with per-message truncation.
...
- `src/runtime-input-builder.ts`:
  - Build model-switch handoff blocks from persisted
    `conversation_handoff_summaries`; keep recent-message blocks only for
    non-switch resume/recovery fallback.
...
- Tests:
  - Codex non-switch soft-inject tests should assert bounded recent messages can
    be injected and `CLAUDE.md` is not prepended;
  - model-switch tests should assert the persisted handoff summary is injected
    instead of raw recent messages;
