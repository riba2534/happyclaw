...
const CODEX_MODEL_PRESETS = ['gpt-5.4', 'gpt-5.4-mini', 'gpt-5.3-codex'] as const;
...
export function ContainerEnvPanel({ groupJid, onClose }: ContainerEnvPanelProps) {
  ...
  useEffect(() => {
    if (!config) return;
    setRuntime(config.runtime || 'claude');
  ...
  const handleSave = async () => {
    ...
    if (openaiApiKeyDirty) data.openaiApiKey = openaiApiKey;
    ...
    if (runtime === 'codex') data.codexModel = codexModel.trim();
    ...
    <div className="flex flex-col h-full min-h-0">
      ...
      <div className="flex-1 min-h-0 overflow-y-auto px-4 py-3 space-y-4">
        <p className="text-[11px] text-muted-foreground leading-relaxed">
          覆盖全局 Agent 运行时配置，仅对当前工作区生效。留空则使用全局配置。保存后工作区将自动重建。
        </p>
          ...
          <div>
            ...
            <div className="flex gap-2">
              ...
              <Button
                type="button"
                size="sm"
                variant={runtime === 'codex' ? 'default' : 'outline'}
            ...
            <>
              ...
              <div>
                ...
                <Input
                  ...
                  placeholder={config?.hasOpenAIApiKey ? '已设置，输入新值覆盖；留空可清除覆盖' : '留空使用全局配置'}
                  className="px-2.5 py-1.5 text-xs h-auto"
                />
              </div>
