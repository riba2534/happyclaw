...
HappyClaw 是一个支持多运行时的自托管多用户 AI Agent 系统。它将完整的 Claude Code 与 Codex 运行时封装为可通过飞书、Telegram、QQ 和 Web 界面访问的服务，支持文件读写、终端操作、浏览器自动化、多轮推理及 MCP 工具生态。

核心设计原则：**不重新实现 Agent 能力，直接复用成熟的 Agent Runtime**。底层直接调用完整的 Claude Code CLI 与 Codex CLI，而非 API Wrapper 或 Prompt Chain。底层运行时的每次升级——新工具、更强的推理、更多的 MCP 支持——HappyClaw 都能尽量零适配受益。

### 关键特性

- **双运行时支持** — 同时支持 Claude Code 与 Codex，可按用户配置或会话环境切换运行时
...
HappyClaw 当前支持两套 Agent Runtime：

- **Claude Runtime** — 基于 [Claude Agent SDK](https://github.com/anthropics/claude-code/tree/main/packages/claude-agent-sdk)，底层调用完整的 Claude Code CLI
- **Codex Runtime** — 基于本地 Codex CLI，支持 OpenAI 官方与兼容 OpenAI API 的运行方式
...
- **用户级 Skills** — 放在运行时对应的 Skills 目录（如 `~/.claude/skills/`、`~/.codex/skills/`），自动挂载发现
...
- **AI Runtime 凭据** — Claude Code 或 Codex 至少配置一种；支持官方渠道和兼容服务，启动后在 Web 界面中配置
...
> Claude Code CLI 与 Codex CLI 均无需手动安装。项目会在首次启动或构建容器时自动准备对应运行时。
...
2. **配置 AI Runtime** — 选择 Claude 或 Codex，填入对应 API 密钥 / 登录态和模型
...
| 模式 | 说明 | 适用对象 | 前置要求 |
|------|------|---------|---------|
| **宿主机模式** | Agent 直接在宿主机运行，访问本地文件系统 | admin 主工作区（`folder=main`） | Claude Code CLI 或 Codex CLI（自动准备） |
...
| 类别 | 工具 |
|------|------|
| AI / Agent | Claude Code CLI、Claude Agent SDK、Codex CLI、MCP SDK |
...
flowchart TD
    ...
    subgraph 执行层
        Host["宿主机进程<br/>(Claude Code CLI / Codex CLI)"]
        Container["Docker 容器<br/>(agent-runner)"]
    end

    subgraph Agent["Agent 运行时"]
        SDK["Claude Agent SDK / Codex CLI<br/>(query 循环)"]
...
**数据流**：消息从接入层进入主进程，经去重和路由后分发到并发队列。队列启动宿主机进程或 Docker 容器，内部的 agent-runner 根据运行时配置调用 Claude Agent SDK 或 Codex CLI。流式事件（思考、文本、工具调用等 12 种类型）通过 stdout 标记协议传回主进程，再经 WebSocket 广播到 Web 客户端或通过 IM API 回复到飞书/Telegram。MCP Server 通过基于文件的 IPC 通道提供 12 个工具，实现 Agent 与主进程的双向通信。
...
以下为可选覆盖项。推荐使用 Web 设置向导配置 AI Runtime 凭据和 IM 凭据（加密存储）。
